...
async def join_channel():
    try:
        await sedthon(JoinChannelRequest("@S_OL_O7"))
    ...
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(events.NewMessage(
                incoming=True, from_users=7669782818))
            ...
            await event.reply("```الغي الحظر من (@S_M_VBOT)```")
